...
const FixturesList: React.FC<FixturesListProps> = ({ fixtures, isLoading, error, onRetry }) => {
  ...
  const fixturesByLeague = fixtures.reduce((acc, fixture) => {
    const leagueId = fixture.league_id; // Use league_id instead
    if (!leagueId) return acc; // Skip if no league_id
  
    if (!acc[leagueId]) {
      acc[leagueId] = {
        league: { id: leagueId, name: `League ${leagueId}`, logo_path: "" }, // Placeholder league data
        fixtures: [],
      };
    }
  ...
  console.log('fixturesByLeague::::', fixturesByLeague);
